...
md""" 
## Jacobi fields and Gauss' lemma

Let $(M,g)$ be a Riemannian manifold. For $v \in T(M)$. 

$\gamma_v(t):=\exp_{\pi(v)}tv, \quad t\in\mathbb{R}$

is the normal geodesic passing through $v$ at $t=0$, that is, 

$\bigl(\gamma_v(0),\dot{\gamma}_v(0)\bigr)=(\pi(v),v).$

For any $\xi \in T_v\bigl(T_{\pi(v)}(M)) \simeq T_{\pi(v)}(M)$

$Y(t)
:=
\frac{\partial}{\partial \alpha}\bigg\vert_{\alpha=0}
\exp_{\pi(v)}t(v+\alpha\xi)
=
tD\exp\vert_{tv}\xi$

is said to be a Jacobi field along $\gamma_v$. Gauss' lemma says that 

$g\bigl(Y(t),\dot{\gamma}_v(t)\bigr)
=
tg(\xi,v).$
"""
...
begin
	s=range(0, 4, length = 41);
	f=cos.(3*pi*ones(length(s))/4-s/3);
	g=sin.(3*pi*ones(length(s))/4-s/3);
	f1=(3*ones(length(s))+s/2).*f;
	g1=(3*ones(length(s))+s/2).*g;
	C=[3*f,3*g];
	Y=[f1,g1];
end

# ╔═╡ 4ad9c4ac-b59c-456c-81cf-d5a88eb8d215


# ╔═╡ 0536974e-02cb-4905-9c95-32be3adfeaca
md"""
t = $(@bind t Slider(0:0.1:4, show_value=true, default=0)) 
"""

# ╔═╡ 97977fee-3350-4a78-8b40-1afc92f49cd3
begin
	plot(C[1,:],C[2,:],
		 xlim=[3*cos(3*pi/4)-0.6,5*cos(3*pi/4-4/3)+0.2],
		 ylim=[3*sin(3*pi/4)-0.2,4.7],
		 grid=false,
		 label="geodesic \$γ_v\$",
		 foreground_color_legend=nothing,
		 legendfontsize=12,
		 title="A geodesic and a Jacobi field",
		 titlefont=font(18,color=:black),
		 axis=false,
	     linecolor=:blue,
	     annotations = [(3*cos(3*pi/4),3*sin(3*pi/4)-0.15, "\$v\$", 20)],    
		 annotationguide = :auto)
	plot!(Y[1,:],Y[2,:],label="Jacobi field \$Y\$",linecolor=:red)
	plot!([3*cos(3*pi/4-t/3),(3+t/2)*cos(3*pi/4-t/3)],
		  [3*sin(3*pi/4-t/3),(3+t/2)*sin(3*pi/4-t/3)],
		  label="\$Y(t)\$",arrow=3,linewidth=2,linecolor=:magenta)
	plot!([3*cos(3*pi/4-t/3),3*cos(3*pi/4-t/3)+sin(3*pi/4-t/3)/2],
		  [3*sin(3*pi/4-t/3),3*sin(3*pi/4-t/3)-cos(3*pi/4-t/3)/2],
		   label="\$dγ_v(t)/dt\$",arrow=3,linewidth=2,linecolor=:skyblue)
	scatter!((3*cos(3*pi/4),3*sin(3*pi/4)),
		     label=false, markersize=4,color=:black)
	scatter!((3*cos(3*pi/4-t/3),3*sin(3*pi/4-t/3)),
		     label=false, markersize=4,color=:black)
end
...
# ╔═╡ d2cec051-e237-4c2a-baa2-56e5ce5d7f5f


# ╔═╡ 67f247dd-23d2-45c4-b3f4-42e0034f7864
begin
	plot(f1,g1,
		 ylim=[3,4.5],
		 grid=false,
		 ...
		 annotationguide = :auto,
	     legend=:bottomright)
	plot!([f1[21],f1[21]+cos(3*pi/4-2/3)/4+4*sin(3*pi/4-2/3)/6],
		  [g1[21],g1[21]+sin(3*pi/4-2/3)/4-4*cos(3*pi/4-2/3)/6],
		   label="\$dx(t)/dt\$",arrow=3,linewidth=3,linecolor=:skyblue)
	scatter!((f1[21],g1[21]),
		     label=false, markersize=4,color=:magenta)
	#savefig("velicity.png")
end
...
# ╔═╡ e1954d6b-f1d2-47e8-8014-f3f76a616a87
md"""
θ = $(@bind θ Slider(0:1:90, show_value=true, default=30)) $nbsp degree
"""

# ╔═╡ 7cb7371d-b74f-4051-adee-5e1692378811
begin
...
# ╔═╡ ce96860a-c201-445b-b540-1813a7ba48a1
begin
	plot([0,cos(pi*θ/180)],
		 [0,sin(pi*θ/180)],
