...
md"""
 ...
 altitude = $(@bind latitude Slider(0:2:88, show_value=true, default=6)) deg
 $nbsp $nbsp
...
begin
	...
	r1=1-a1^2-b1^2;
	x1 = range(-1, length=101, stop=1);
	y1 = range(-1, length=101, stop=1);
	f1=zeros(length(x1),length(y1));
	h1=zeros(length(x1),length(y1));
	for k=1:101
		for l=1:101
			f1[k,l]=1-x1[k]^2-y1[l]^2;
			h1[k,l]=1+a1^2+b1^2-2*a1*x1[k]-2*b1*y1[l];
		end
	end
end
...
# ╔═╡ 10609a19-288b-4bc0-a349-0ae5589e9043
begin
    SEC1=plot(f1[:,l0],
		grid=false,
		linewidth=2,
		label="\$z=f(x,b)\$",
	    xticks=([1 51 101;],[-1,0,1]),
	    yticks=([0 1;],[0,1]),
	    xlabel="x",
	    ylabel="z")
	plot!(h1[:,l0],
    ...
    plot(SEC1,SEC2,layout=(1,2),size=(1200,400))
end
...
# ╔═╡ 851efeec-5a98-4e45-8c75-768920354537
md"""
### Newton mathod

$\vec{x}_{n+1}:=\vec{x}_n-\bigl(f^{\prime\prime}(\vec{x}_n)\bigr)^{-1}f^\prime(\vec{x}_n),\quad n=0,1,2,3,\dotsc$
"""
...
md"""
 $s$ = $(@bind s6 Slider(0.01:0.01:0.2, show_value=true, default=0.05))

 $x_0$ = $(@bind a6 Slider(-1:0.1:1, show_value=true, default=0.8)) 

 $y_0$ = $(@bind b6 Slider(-1:0.1:1, show_value=true, default=0.8))

 $n$ = $(@bind n6 Slider(0:1:N5, show_value=true, default=0))
"""
...
begin
    p6=surface(x4,y4,f4,
	        ...
	        legend = :none)
	scatter!([0],[0],[0],color=:yellow,label=false,markersize=4)
	scatter!([Y4[1:n6+1,1]],[Y4[1:n6+1,2]],[Y4[1:n6+1,3]],color=:magenta,label=false,markersize=4)
	...
	q6=surface(x4,y4,f4,
	        ...
	        legend = :none)
	scatter!([0],[0],[0],color=:yellow,label=false,markersize=4)
	scatter!([Y5[1:n6+1,1]],[Y5[1:n6+1,2]],[Y5[1:n6+1,3]],color=:magenta,label=false,markersize=4)

	plot(p6,q6,layout=(1,2),size=(2000,800))
end
