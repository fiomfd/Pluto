...
Let $f:[a,b] \rightarrow [m,M]$ be a real-valued bounded function on an interval $[a,b]$. We define a Riemann sum of $f$. We introduce division of $[a,b]$
...
$a_0,\dotsc,a_n\in[a,b]$ such that $a=a_0<a_1<\dotsb<a_n=b$, $f \in C(a_{i-1},a_i)$, 
$i=1,\dotsc,n$ and limits of finite values 

$f(a_0+0), \quad f(a_1-0), \quad f(a_1+0), \quad \dotsc, \quad f(a_n-0)$
...
begin 
	x = range(0, 1, length = 501);
	f=sin.(2pi*x)+x+1.2*ones(501);
end
...
md"""
n= $(@bind n Slider(1:500, show_value=true))
"""
...
begin
	g=zeros(n);
	for j=1:n
		g[j]=sin.(2pi*(2*j-1)/2/n)+(2*j-1)/2/n+1.2;
	end
end
